...
const IMAGE_HEIGHT = 184; // 시안 고정 치수 (node 53-289). 이미지 없으면 단색 placeholder
const STATUS_DOT_SIZE = 10;
const NOTICE_RADIUS = 12;  // ⚠️ 토큰 없음 — Figma 12 (radius.sm=8·card=16 사이). 디자이너 확정 시 통합
...
export default function ItemDetail({ item }: { item: Item }) {
  ...
  return (
    <View>
      {/* 2. 상단 히어로 이미지(풀블리드). 있으면 영역을 꽉 채우고, 없으면 단색 placeholder 유지 */}
...
const styles = StyleSheet.create({
  image: { // 맨 위 히어로 이미지 영역(풀블리드, 모서리 둥글림 없음). 이미지 없으면 단색
    width: '100%',
    height: IMAGE_HEIGHT,
    backgroundColor: colors.surface, // 이미지 로드 전/없을 때 보이는 단색 placeholder
    overflow: 'hidden', // 안의 이미지가 영역 밖으로 삐져나오지 않게
  },
  ...
  storage: { ...typography.body, color: colors.textTertiary }, // 이름 오른쪽 보관방식 글자(냉장/냉동/실온). 시안 연한 회색
  ...
  noticeBox: { // '참고용이에요' 안내 박스 (시안: 초록 테두리 박스)
    borderWidth: 1,
    borderColor: colors.primary,
    borderRadius: NOTICE_RADIUS,
    padding: spacing.md,
  },
  noticeText: { ...typography.caption, color: colors.textPrimary, lineHeight: 18 }, // 안내 박스 안 글자
