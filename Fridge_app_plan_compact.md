...
> **기준일: 2026-06-15 (표준 재료 대표 이미지 도입: ingredient_master.image_path + Storage 공개 URL 헬퍼 + 메인/상세/등록 그리드 3화면 표시 / UX/UI 3차: 냉장고 행·친구 카드 통일 + 등록 화면 Figma+휠 피커 + 레시피 목록/작성 Figma·토큰화 / 2차: 토큰 정비 + 탭바 활성표시 + 커뮤니티 화면 Figma 적용 / 1차: 라우팅 재편·냉장고 필터·정렬·등록 통합)**
...
| ✅ | **표준 재료 대표 이미지(§21)** — `ingredient_master.image_path`(파일명) + public Storage 버킷 `ingredient-images`(183장, 296행 매핑) + `ingredientImageUrl` 공개 URL 헬퍼. 메인(`FridgeItemRow` 원형 50)·재료정보(`ItemDetail` 상단 184 히어로)·등록 그리드(`register/ingredient` 원형 썸네일) 3화면 표시. items는 `ingredient_master(image_path)` 임베드를 조회 시 평탄화 |
| 🟡 | **UX/UI 디자인 적용** — 토큰화·주요 화면 레이아웃 적용 진행 중. 남은 일: ⚠️ 추정 토큰값(텍스트/보더/틴트/타이포)·시안 고정값(라운드/그림자/폰트크기) 디자이너 확정값으로 교체, 나머지 화면(item·fridge·mypage 등) Figma 적용, mypage 등 raw 숫자 화면 토큰화 |
| 보류 | 푸시 알림(Expo Notifications), 사용자 정의 카테고리, **사용자 업로드 식재료 사진**(현재는 표준 마스터 대표 이미지만 §21. 직접 입력/이미지 없는 마스터는 단색 placeholder) |
...
> 이미지 컬럼 없음. 대표 이미지는 `lib/items.ts`의 `fetchMyItems`/`fetchItem`이 `select('*, ingredient_master(image_path)')` 임베드로 가져와 **조회 시 `item.image_path`로 평탄화**(미연결/이미지 없음=null). `Item` 타입에 `image_path: string|null` 존재(§21).
...
**recipes**: `id` · `owner_id` · `title` · `body` · `source(manual|ai)` · `ai_meta(jsonb, nullable)` — AI 레시피만 `{cook_time_minutes, difficulty, needed_ingredients[{name,amount}]}`, 수동은 null
...
`id` · `name(UNIQUE)` · `category` · `room_days` · `fridge_days` · `freezer_days` · `storage_tip` · `image_path(text, nullable)` · `created_at` — **읽기 전용 마스터**(앱에서 수정 불가, 적재는 SQL Editor/service role)
- `room_days/fridge_days/freezer_days = 0` → 해당 보관방식 비권장
- `image_path` = Storage 버킷 `ingredient-images` 내 **파일명만**(경로 없이, 예 `ingredient_001.jpg`). public URL 변환은 `ingredientImageUrl`(§21)
...
components/
  ...
  ItemDetail.tsx      # 재료 정보 읽기 전용 표시(상단 대표 이미지 184·D-day 상태점·보관팁·안내문구). 이미지=`item.image_path`→`ingredientImageUrl`, contain·흰 배경, 없으면 단색(§21)
  FridgeItemRow.tsx   # ★냉장고 목록 한 행(§17·§20). 원형 썸네일(50, 이미지 cover·없으면 단색 §21)·이름·D-day(bold)·임박 색점(이름 옆 인라인)·"날짜까지-보관"·우측 `›` 셰브론. 냉장고/친구 냉장고 공용
...
lib/    supabase · items · profiles · friends · comments · notifications · recipes · expiry · format · ingredients · account
        ...
        - ingredients.ts: fetchIngredientsByCategory · fetchIngredientById · searchIngredients(이름 검색) · **ingredientImageUrl**(image_path→public URL, null/빈값이면 null §21) · 상수 INGREDIENT_IMAGES_BUCKET='ingredient-images'
...
- 출력(확장 스키마, 6필드): `{ title, body, cook_time_minutes(정수), difficulty(1~5 정수), used_ingredients[], needed_ingredients[{name, amount}] }`
- 프롬프트 규칙: used=냉장고 재료 중 실제 사용분 / needed=냉장고에 **없는** 추가 재료만(amount는 g·ml·개·큰술 등 한국 단위), cook_time 정수, difficulty 1~5, body 단계별
- `used_ingredients`는 **저장·미리보기·상세 어디에서도 미표시**(디자인 결정). 서버 응답에는 포함되나 클라이언트는 사용 안 함
- 파싱 방어: `difficulty` `typeof==='number' && isFinite`일 때만 사용(아니면 기본 3) 후 1~5 클램프·정수화 / `cook_time_minutes` `Number()` 관대 처리, 숫자 아니면 0 / `needed_ingredients` 배열 아니면 `[]`, 각 원소 `{name, amount}` 정규화 / **needed에서 입력 재료명과 공백제거·소문자 일치 항목은 서버에서 재차 제거**(중복 방지) / `title`·`body` 누락 시 500
- 타임아웃 **30초**(AbortController, `TIMEOUT_MS=30_000`). 실패/파싱오류 시 500 + 메시지. API 키는 로그에 남기지 않음

**저장 (recipes.ai_meta)**
- `recipes` 테이블에 `ai_meta(jsonb, nullable)` 컬럼 추가. AI 메타 = `{ cook_time_minutes, difficulty, needed_ingredients[{name, amount}] }`
- AI 레시피 저장: `title`/`body`/`source='ai'` + `ai_meta`에 위 3필드. 수동 레시피는 `ai_meta=null`
- `createRecipe(userId, values, source='manual', aiMeta=null)`로 확장(후방 호환 — 수동 호출 그대로 동작)
- ⚠️ `ALTER TABLE ADD COLUMN`은 기존 RLS 정책/GRANT가 새 컬럼을 그대로 커버 → **재설정 불필요**

**클라이언트**
- `lib/recipes.ts`: `requestAiRecipe(items)` — `supabase.functions.invoke('recommend-recipe')`로 호출(JWT 자동 첨부). `createRecipe`는 4번째 인자 `aiMeta` 추가(위 참조)
- `types/recipe.ts`: `AiRecipeResult` + `NeededIngredient { name, amount }` + `RecipeAiMeta { cook_time_minutes, difficulty, needed_ingredients[] }` + `Recipe.ai_meta(RecipeAiMeta|null)`
- `app/(main)/recipes/new.tsx`: 모드 분기(직접 작성 / AI 추천). 흐름: 냉장고 재료 fetch(임박 순, 기본 선택) → 추천받기 → 미리보기 → 저장(source='ai', ai_meta 포함) / 다시 추천
  - **미리보기 Figma(node 1-469)**: 제목 + `🕐 {cook_time}분 l 난이도 ●●●○○` + body + 구분선 + "부족한 재료" 칩. 누락 방어(`needed_ingredients ?? []`, difficulty 기본3·1~5클램프, cook_time 숫자 아니면 `-`). CTA "추가하기"(primary pill) + "다시 추천"(보조)
- `app/(main)/recipes/[recipeId].tsx` 상세 **Figma(node 1-410)**: AI 레시피(`source='ai' && ai_meta`)는 시간/난이도(점)/부족재료(칩)/body 표시(미리보기와 동일 방어 패턴), 수동 레시피는 `title/생성일/body`만. 수정·삭제 topBar·편집 모드 유지
  - ⚠️ **레거시 폴백**: `source='ai'`이지만 `ai_meta=null`인 과거 레시피는 상세에서 **수동 뷰로 폴백**(크래시 방지)
- 색·radius·간격은 `constants/theme.ts` 토큰 매칭, 미존재 폰트크기 등은 리터럴(리콘사일 후보). 미리보기·상세의 칩/난이도점 스타일은 현재 **복제**(공용 컴포넌트 추출은 후속)
- ⚠️ 스키마 선행 주의: 클라이언트 타입을 먼저 새 스키마로 바꾸면 Edge Function 재배포 전까지 `needed_ingredients` 누락으로 `.map()` 런타임 크래시. **서버(Edge Function) 먼저 수정·`supabase functions deploy recommend-recipe` 재배포 후 클라이언트**
...
| Gemini API 호출 타임아웃 | 레시피 생성은 응답이 길어 10초가 빠듯. 현재 `TIMEOUT_MS=30_000`(30초) 적용 |
| 앱에서 "Edge Function returned a non-2xx status code"만 뜨고 원인 불명 | 대시보드 Edge Functions > Logs에서 함수의 `console.error` 메시지로 실제 원인 확인 |
| `Cannot read properties of undefined (reading 'map')`(미리보기/상세) | 클라이언트 타입만 새 스키마로 앞서가고 Edge Function이 구 스키마 반환 → `needed_ingredients` undefined. **스키마 확장은 서버(Edge Function) 먼저 수정·재배포 → 그다음 클라이언트** 순서. 더해 렌더부에 `?? []` 방어 |
| 함수 코드 고쳤는데 응답이 그대로(구 스키마) | `supabase functions deploy recommend-recipe` 재배포 누락. 로컬 파일 수정만으로는 반영 안 됨 |
...
## 8-8. §21 표준 재료 대표 이미지 — Storage 연동 + 3화면 표시 (구현 완료)

`ingredient_master`에 대표 이미지를 붙여 메인·재료정보·등록 그리드에서 표시. **사용자 업로드가 아니라 앱 제공 표준 마스터 이미지**(읽기 전용).

**데이터·인프라 (Supabase, 수동 준비 완료)**
- `ingredient_master`에 `image_path text` 컬럼 추가. 값은 **파일명만**(경로 없이, 예 `ingredient_001.jpg`).
- public Storage 버킷 **`ingredient-images`** 생성, 이미지 **183장** 업로드. 296개 재료 행에 `image_path` 매핑(여러 재료가 한 장 공유 가능).
- 적재 SQL: `NangBu_이미지_적용.sql`(컬럼 추가 + `update ... set image_path` 매핑). public URL 도달 확인됨(HTTP 200, image/jpeg).

**공통 베이스 (1단계)**
- `types/ingredient.ts`: `IngredientMaster`에 `image_path: string | null` 추가.
- `lib/ingredients.ts`: 상수 `INGREDIENT_IMAGES_BUCKET = 'ingredient-images'` + 헬퍼 **`ingredientImageUrl(imagePath)`** — `supabase.storage.from(버킷).getPublicUrl(path).data.publicUrl` 반환, **null/공백이면 null**(화면에서 placeholder 분기용). 조회 함수의 select는 `*` 유지(컬럼 추가만으로 내려옴, 불필요 diff 회피).

**items 경로 연동 (2단계 — 메인/상세용)**
- items에는 이미지 컬럼이 없고 `ingredient_id`(nullable FK)만 있음. `lib/items.ts`의 `fetchMyItems`/`fetchItem` 두 곳만 `select('*')` → **`select('*, ingredient_master(image_path)')`** 임베드.
- 내부 `flattenItem(row)`로 **평탄화**: 임베드 객체의 `image_path`를 끌어올려 `item.image_path`로 노출하고 `ingredient_master` 키 제거. `ingredient_id`가 null(직접 입력)이거나 마스터에 이미지 없으면 `image_path: null`. 반환 타입은 기존 `Item[]`/`Item` 유지(별도 뷰 타입 안 만듦). `createItem`/`updateItem`의 `.select()`는 미변경(폼 응답에 이미지 불필요).
- `types/item.ts`: `Item`에 `image_path: string | null` 추가.

**화면 표시 (3·4·5단계 — 동일 패턴: `const uri = ingredientImageUrl(...)`, uri 있으면 `<Image>`, null이면 기존 단색 유지·placeholder 신설 금지)**
- **메인 `FridgeItemRow.tsx`**: 원형 썸네일(50×50, radius 25) 안에 `<Image style={{w/h:'100%'}} resizeMode="cover">`, `styles.thumb`에 `overflow:'hidden'`. 작은 원형이라 **cover**(꽉 채움, 약간 크롭).
- **재료정보 `ItemDetail.tsx`**: 상단 히어로 영역. `IMAGE_HEIGHT` 200→**184**(Figma node 1-937), `borderRadius: radius.sm(8)`, `overflow:'hidden'`. **resizeMode `contain`**(세로로 긴 가지 등이 위아래 잘리던 문제 → 전체 보이게) + 여백 배경 `colors.background`(흰색, 초록 대신). `colors.thumbnail` 토큰 자체는 미변경(메인 썸네일 기본색 공유).
- **등록 그리드 `register/ingredient.tsx`**: `renderItem` 인라인. 원형 썸네일(동적 크기, 최대 72) 안에 `<Image cover>`, `styles.thumb`에 `overflow:'hidden'`. 선택 테두리(`thumbSelected`)·체크 오버레이(`checkOverlay`)는 형제라 그대로 위에 겹침. 마스터 직접 조회라 `ingredient.image_path` 바로 접근(items 조인 불필요).

**cover/contain 결정**: 작은 원형 썸네일(메인·등록)=cover, 큰 사각 히어로(재료정보)=contain. 영역 형태에 따라 분기.

**검증**: 각 단계 `npx tsc --noEmit`에서 변경 파일 에러 0(기존 `supabase/functions/recommend-recipe/index.ts`의 Deno 전역 미해결 5건은 RN tsconfig에 Deno 타입 부재로 인한 사전 존재 에러 — 무관).
